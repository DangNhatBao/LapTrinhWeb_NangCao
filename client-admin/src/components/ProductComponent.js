  ...
  render() {
    ...
    const pagination = Array.from({ length: this.state.noPages }, (_, index) => {
      if ((index + 1) === this.state.curPage) {
        return (
          <span key={index} className="mx-1 text-gray-900 font-bold">
            | {index + 1} |
          </span>
        );
      } else {
        return (
          <span
            key={index}
            className="mx-1 text-gray-900 hover:underline cursor-pointer"
    ...
    return (
      <div className="p-4">
        <div className="w-full lg:w-2/3 float-left">
          <h2 className="text-center text-2xl mb-4 font-bold">DANH SÁCH SẢN PHẨM</h2>
